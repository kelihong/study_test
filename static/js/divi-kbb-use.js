let urlData = "http://192.168.88.200:8082/api/goods/listGoodsCompositeGraph";
...
function create3DBox({ urlData, curImageHost, urlResult } = {}) {
  ...
  function createAppointIndexSwiperImage(index, url) {
    let imgUrlList = [
      curImageHost + kbbnice.styleList[index].texture,
      //   curImageHost + kbbnice.styleList[index].image,
    ];

    // 如果有传入 cover 图，则渲染 cover 图
    if (url) {
      // 获取到定位点，将图片变形
      // let canvasParams = JSON.parse(kbbnice.styleList[index].goodsPagesPartVoList[0].canvasConfig)
      let canvasModel = kbbnice.styleList[index].goodsPagesPartVoList;
      // 变形图片
      setTransformImage();
